...
class PosSession(models.Model):
    ...
    # overrride for customization of combine account payment
    def _create_combine_account_payment(self, payment_method, amounts, diff_amount):
        ...
        if not account_payment.outstanding_account_id and accounting_installed:
            account_payment.outstanding_account_id = (
                account_payment._get_outstanding_account(account_payment.payment_type)
            )
        ...
        if payment_method.parent_payment_method_name:
            ...
            if not journal:
                raise UserError(
                    _("Please configure a Journal for bank transaction charge.")
                )

            commission_percent = (
                getattr(journal, "commission_tax_percentage", 0.0) or 0.0
            )
            commission_fixed = getattr(journal, "commission_tax_fix", 0.0) or 0.0

            total_difference = (
                (amounts["amount"] * commission_percent / 100.0)
                + commission_fixed
                + diff_amount
            )
            ...
            outstanding_line = move_lines.filtered(
                lambda line: line.account_id == outstanding_account
            )[0]
            if not outstanding_line:
                raise UserError(
                    _("Could not find outstanding account line in payment move.")
                )
            ...
            if outstanding_line.debit:
                vals["debit"] = max(
                    (
                        outstanding_line.debit - total_difference
                        if outstanding_line.debit
                        else 0.0
                    ),
                    0.0,
            ...
            if outstanding_line.credit:
                vals["credit"] = max(
                    (
                        outstanding_line.credit - total_difference
                        if outstanding_line.credit
                        else 0.0
                    ),
                    0.0,
